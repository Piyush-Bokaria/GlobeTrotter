...
const ItineraryBuilder = () => {
  ...
  const fetchItinerary = async () => {
    try {
      const response = await fetch(`http://localhost:5000/apis/itinerary/${tripId}`);
  ...
  const createItinerary = async () => {
    try {
      const response = await fetch('http://localhost:5000/apis/itinerary/create', {
        method: 'POST',
        headers: {
  ...
  const addDay = async (dayData) => {
    try {
      const response = await fetch(`http://localhost:5000/apis/itinerary/${tripId}/days`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const addActivity = async (dayId, activityData) => {
    try {
      const response = await fetch(`http://localhost:5000/apis/itinerary/${tripId}/days/${dayId}/activities`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const updateActivity = async (dayId, activityId, activityData) => {
    try {
      const response = await fetch(`http://localhost:5000/apis/itinerary/${tripId}/days/${dayId}/activities/${activityId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        const data = await response.json();
        setItinerary(data.itinerary);
        setEditingActivity({ show: false, dayId: null, activityId: null, data: {} });
      } else {
        const errorData = await response.json();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="px-4 py-6 sm:px-0">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-gray-600">
                  Plan your trip day by day with detailed activities and schedules
                </p>
              </div>
              {itinerary && (
                <div className="text-right">
                  <p className="text-sm text-gray-500">Total Days: {itinerary.totalDays}</p>
                  <p className="text-sm text-gray-500">Total Budget: ${itinerary.totalBudget}</p>
                </div>
              )}
            </div>
          </div>
          ...
          {!itinerary ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">No itinerary yet</h3>
            ...
            <div className="space-y-6">
              ...
              {itinerary.days.length === 0 ? (
                <div className="text-center py-8 bg-white rounded-lg shadow">
                  <p className="text-gray-500">No days added yet. Click "Add Day" to start planning!</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {itinerary.days.map((day, dayIndex) => (
                    <div key={day._id} className="bg-white rounded-lg shadow-lg overflow-hidden">
                      {/* Day Header */}
                      <div className="bg-indigo-50 px-6 py-4 border-b border-indigo-100">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              onClick={() => setShowAddActivity({ show: true, dayId: day._id })}
                              ...
                              <div key={activity._id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                                <div className="flex items-start justify-between">
                                  <div className="flex-1">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                                      <div>
                                        ...
                                        {activity.cost > 0 && <p><strong>Cost:</strong> ${activity.cost}</p>}
                                      </div>
                                      <div>
                                  ...
                                  <div className="flex items-center space-x-2 ml-4">
                                    ...
                                    <button
                                      onClick={() => deleteActivity(day._id, activity._id)}
      ...
      {/* Add Day Modal */}
      {showAddDay && <AddDayModal onClose={() => setShowAddDay(false)} onAdd={addDay} />}
      ...
      {editingActivity.show && (
        <EditActivityModal 
          ...
          onClose={() => setEditingActivity({ show: false, dayId: null, activityId: null, data: {} })} 
          onUpdate={(data) => updateActivity(editingActivity.dayId, editingActivity.activityId, data)} 
        />
      )}
    </div>
  );
